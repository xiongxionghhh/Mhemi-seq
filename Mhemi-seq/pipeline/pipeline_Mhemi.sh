...
echo -e '\nAttention: This script is used only for MspJI cut and 2bp UMI adjacent to adapter!\n'
...
if [[ -z ${reference_genome} ]]; then
	...
	for prefix in `ls ./fastq/*_R1.fastq.gz | sed "s/_R1.fastq.gz//g; s/\.\/fastq\///g" | sort --parallel ${nthread} -u `; do
		...
		fastp --umi --umi_loc per_read --umi_len 2 --json tmp.${prefix}.json --html tmp.${prefix}.QC.html -q 20 -e 20 -l 30 -w ${nthread} --in1 ../fastq/${prefix}_R1.fastq.gz --out1 tmp.${prefix}.QC.1.fq.gz --in2 ../fastq/${prefix}_R2.fastq.gz --out2 tmp.${prefix}.QC.2.fq.gz 2> /dev/null
		...
		samtools view -@ ${nthread} tmp.dup.bam | awk '{print substr($1,1,length($1)-6), $0}' OFS='\t' | cut -f 1,3- | cat tmp.header - | samtools view -hb -@ ${nthread} | samtools sort -m 2G -@ ${nthread} > tmp.dup2.bam
		...
		samtools sort -n -m 2G -@ ${nthread} tmp.dup.bam | bedtools bamtobed -bedpe -i - | awk '{umi=substr($7,length($7)-4,5)} {if(($9=="+") && ($10=="-")) print $1, $2, $6, umi, $7; else if(($9=="-") && ($10=="+")) print $4, $5, $3, umi, $7}' OFS='\t' > tmp.unsort.bed
